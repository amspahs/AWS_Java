...
public class ArenaRule {
	...
	public static int getCardToJumsu(Card[] cardArr) {
		...
		for (int i = 0; i < cardArr.length; i++) {
			card = cardArr[i].getCard();

			String numberStr = card.substring(1);

			if ("A".equals(numberStr)) {
				scoreSum = scoreSum + 1;
			} else if ("T".equals(numberStr)) {
				scoreSum += 10;
			} else if ("J".equals(numberStr)) {
				scoreSum = scoreSum + 11;
			} else if ("Q".equals(numberStr)) {
				scoreSum += 12;
			} else if ("K".equals(numberStr)) {
				scoreSum += 13;
			} else {
				scoreSum = scoreSum + Integer.parseInt(numberStr);
			}
		}
